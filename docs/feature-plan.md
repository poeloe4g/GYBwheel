...
- **Milestone:** M0 · **Priority:** P0 · **Size:** S · **Status:** TODO
- **Depends on:** —
...
- **Acceptance criteria:**
  - [ ] Repo installs cleanly into a fresh venv.
  - [ ] `python main.py --help` runs and prints usage (even if no logic yet).
  - [ ] Pinned deps: `pandas`, `requests`, `pyyaml`, `python-dotenv`, `py_vollib` (fallback), `pytest`.
...
- **Acceptance criteria:**
  - [ ] `config.yaml` includes the corrected fields (`avoid_earnings_before_expiry`,
        `risk_free_rate`, `score_denominator_floor`, `scoring.mode`, `data.universe_refresh_days`,
        `data.cache_dir`).
  - [ ] Missing/empty `TRADIER_TOKEN` raises a clear, actionable error.
  - [ ] No secret ever read from `config.yaml`.

### F03 — Abstracted, cached data layer (B7)
- **Milestone:** M1 · **Priority:** P0 · **Size:** L · **Status:** TODO
- **Depends on:** F02
...
- **Acceptance criteria:**
  - [ ] Tradier chain calls request `greeks=true` and parse delta/IV.
  - [ ] yfinance calls are cached per-day and retried with backoff on 429.
  - [ ] Cache hit avoids any network call; cache dir is configurable.

### F04 — Nearest-delta put selector (B5)
- **Milestone:** M1 · **Priority:** P0 · **Size:** M · **Status:** TODO
- **Depends on:** F03
...
- **Acceptance criteria:**
  - [ ] Negative put deltas handled via `abs(delta)`.
  - [ ] Picks the expiry inside `[dte_min, dte_max]`; returns `None` if none qualify.
  - [ ] BS fallback matches greeks-feed delta within tolerance on a fixture.

### F05 — Universe builder & ban list (Spec 1.1)
- **Milestone:** M2 · **Priority:** P0 · **Size:** M · **Status:** TODO
- **Depends on:** F03
...
- **Acceptance criteria:**
  - [ ] A known mega-cap passes; an unprofitable/banned name is dropped with a logged reason.
  - [ ] Universe cache reused within `universe_refresh_days`.

### F06 — Earnings filter (B2)
- **Milestone:** M2 · **Priority:** P0 · **Size:** S · **Status:** TODO
- **Depends on:** F04
...
- **Acceptance criteria:**
  - [ ] A contract spanning earnings is rejected (toggle off → accepted).
  - [ ] Missing earnings data degrades gracefully (warn, don't crash).
...
- **Acceptance criteria:**
  - [ ] Each filter rejects a crafted failing fixture and passes a good one.
  - [ ] Implied-move and yield formulas match hand-computed values (unit test).

### F08 — Account-aware sizing (B1, B3, Spec 1.4)
- **Milestone:** M3 · **Priority:** P0 · **Size:** L · **Status:** TODO
- **Depends on:** F07
...
- **Acceptance criteria:**
  - [ ] With `positions.yaml` absent, treated as empty and header says so.
  - [ ] A name too big for the per-name cap is shown **with** the breach flag + min account size.
  - [ ] Sector/deployed headroom respected; `max_contracts` computed correctly.

### F09 — Regime light (B8, Spec 1.5)
- **Milestone:** M3 · **Priority:** P0 · **Size:** M · **Status:** TODO
- **Depends on:** F03
...
- **Acceptance criteria:**
  - [ ] Forced inputs produce the expected light (unit tests for each case).
  - [ ] RED short-circuits the pipeline (no screening, manage-only message).

### F10 — Scoring & ranking (B4, Spec 2.6)
- **Milestone:** M4 · **Priority:** P1 · **Size:** S · **Status:** TODO
- **Depends on:** F08
...
- **Acceptance criteria:**
  - [ ] Denominator floor prevents blow-ups on near-zero implied move.
  - [ ] Output rows carry each component, not just the blended number.

### F11 — Report output (Spec 2.7)
- **Milestone:** M4 · **Priority:** P1 · **Size:** M · **Status:** TODO
- **Depends on:** F10
...
- **Acceptance criteria:**
  - [ ] CSV columns match the spec's row format.
  - [ ] Console table is readable; header reflects sizing state.

### F12 — Pipeline orchestration & `--paper` (B9, Spec 2.4)
- **Milestone:** M5 · **Priority:** P0 · **Size:** M · **Status:** TODO
...
- **Acceptance criteria:**
  - [ ] RED regime stops before screening.
  - [ ] End-to-end run on ~3 known names produces a sane CSV.
...
- **Acceptance criteria:**
  ...
  - [ ] Coverage includes selector, each quality filter, the B1 breach flag, earnings filter,
